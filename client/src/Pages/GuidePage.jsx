...
const GuidePage = () => {
  ...
  // Use react-responsive for mobile detection
  ...
  // Update gesture service when mobile state changes
  useEffect(() => {
    // If gesture service is available, update its mobile mode
    if (gestureService && typeof gestureService.setMobileMode === 'function') {
  ...
  useEffect(() => {
    ...
    axios.get(API_ENDPOINT + "/guides/" + guideId + "/steps")
      ...
      .catch(err => {
        console.error(err);
        // prevents page from being added to history stack
        navigate('/error', { replace: true });
      })
  }, [guideId, navigate]);

  // Check camera permissions when gestures are enabled
  useEffect(() => {
    ...
    // Only check permissions if gestures are enabled
    if (gesturesEnabled) {
  ...
  useEffect(() => {
    // Dynamic import to check if MediaPipe is available
    import("@mediapipe/tasks-vision")
      .then(({ FilesetResolver }) => {
        console.log("MediaPipe tasks-vision loaded successfully");
      })
      .catch(err => {
        console.error("Error preloading MediaPipe:", err);
      });
  }, []);
  ...
  }, [isTransitioning, navigationCooldown, steps.length]);
  ...
  }, [isTransitioning, navigationCooldown]);
  ...
  const handleGestureDetected = useCallback((gesture, confidence) => {
    ...
    // Only process gestures if not in transition
  ...
  }, [goToNextStep, goToPrevStep, steps.length, isTransitioning, navigationCooldown]);
  ...
  return (
    steps.length > 0 ?
      <div className="bg-[#F8F8F8] text-black w-full overflow-x-hidden">
        ...
        {/* Guide Content with Side Navigation Arrows and gesture control */}
        <section className="w-full py-12 px-6 sm:px-12 bg-white min-h-[60vh] relative">
          <div className="max-w-4xl mx-auto relative">
            {/* Left arrow - fixed on the side for desktop */}
            ...
            {/* Right arrow - fixed on the side for desktop */}
            ...
            <div className="mb-36 sm:mb-16 min-h-[400px] relative overflow-hidden">
                        ...
                        {steps.map((_, index) => (
                          <button
                            ...
                            className={`w-2 h-2 rounded-full transition-all duration-300 ${
                              currentStep === index ? "bg-[#1A3D61]" : "bg-gray-300"
                            }`}
              ...
              {isMobile && (
                <div className={`fixed bottom-20 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-10 transition-transform duration-300 ${
                      ...
                      <div className="max-h-48 overflow-y-auto">
                        {steps.map((step, index) => (
                          <button
                            ...
                            className={`w-full text-left p-2 mb-1 rounded text-sm ${
                              currentStep === index 
                                ? "bg-[#1A3D61] text-white" 
